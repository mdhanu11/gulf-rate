...
export default function MobileRateUpdate() {
  ...
  const { data: providers = [] } = useQuery({
    queryKey: ['/api/countries/sa/providers'],
    queryFn: async () => {
      const response = await axios.get('/api/countries/sa/providers');
  ...
  const filteredRates = rates.filter((rate: Rate) =>
    rate.providerName.toLowerCase().includes(searchQuery.toLowerCase())
  );
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3 flex-1">
                      <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center">
                        {rate.providerKey.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="font-semibold">{rate.providerName}</h3>
                      ...
                      {editingId === rate.id ? (
                        ...
                        <div className="flex items-center gap-4 w-full sm:w-auto">
                          <div className="text-right flex-1 sm:flex-initial">
                            <div className="text-lg font-bold">{rate.rate.toFixed(3)}</div>
                            <div className="text-sm text-gray-600">
                              Fee: {rate.fees} {rate.feeType === 'percentage' ? '%' : 'SAR'}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            {rate.rateChange !== 0 && (
                              <Badge variant={rate.rateChange > 0 ? "destructive" : "secondary"}>
                                ...
                                {Math.abs(rate.rateChange).toFixed(3)}
                              </Badge>
                            )}
